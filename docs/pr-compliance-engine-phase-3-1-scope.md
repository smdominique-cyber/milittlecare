...
fix-target table. For each of the 52 registry rows (organized by
fix-surface), the doc specifies the plain-language guidance and the
deep-link target. Rows where CC is not confident about regulatory
...
source detail to upload a DHS-198) are sub-work in §4.

| Key | Buckets | Guidance | Fix target | Review |
|---|---|---|---|---|
| `funding_dhs_198_on_file` | `missing_required` / `expired` | "Upload the signed DHS-198 form for this CDC funding source. Required for CDC billing." | → Families (funding tab) | NEEDS SETH REVIEW. |
...
Total registry rows mapped: **52** (one-to-one with `REQUIREMENT_REGISTRY`,
modulo the deferred religious-objection row noted in Phase 1 §6).
...
| G — Funding sources + docs | 4 | → Families (funding tab) — works today | 4 |
| H — Attendance acks | 1 | → /acknowledgments | 1 |
| I — Not-yet-shipped (informational) | 12 | No fix target | 0 (copy is locked from Phase 3) |
| **Total** | **52** | | **40 review-flagged** |

**The 40 NEEDS-SETH-REVIEW rows are the gate on actual build.** The
...
1. **Per-requirement guidance accuracy (§2 — the load-bearing
   ...
   consultant review the 40 flagged rows in batches over 1-2
   sessions; confirmed rows ship; unconfirmed rows render
   text-only fallback until they're confirmed. Default if Seth
   wants the simplest path: defer all 40 to "text-only fallback"
   and ship Phase-3-confirmed rows only (the BusinessInfo "What
   applies" rows from Phase 3 are already Seth-confirmed).
...
1. **§2** — the 40 NEEDS-SETH-REVIEW guidance rows. Each one is a
   small batch question: "is this guidance copy regulatorily
   accurate and the right voice for a provider?" Recommend Seth
   review in batches over 1-2 sessions; confirmed rows ship,
   unconfirmed fall through to text-only fallback.
